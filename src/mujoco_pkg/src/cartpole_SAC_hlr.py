...
class deep_learning:
   ...
   def update(self, experience_repository):
      ...
      new_actions, log_prob = self.actor.forward(state_batch)
      entropy = -log_prob
   ...
   def load_model(self):
      self.actor = torch.load("actor.pt", weights_only=False)
      #self.critic_1 = torch.load("critic_1.pt", weights_only=False)
      #self.critic_2 = torch.load("critic_2.pt", weights_only=False)
      with torch.no_grad():
         self.target_critic_1.load_state_dict(self.critic_1.state_dict())
         self.target_critic_2.load_state_dict(self.critic_2.state_dict())
      self.actor.eval()
   ...
   with mujoco.viewer.launch_passive(m, d) as viewer:
      for episode in range(EPISODES):
         ...
         while done == 0:
            ...
            action = DL.select_action_test(state)
            d.actuator('cart_motor').ctrl = action[0] * 500
